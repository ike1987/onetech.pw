...
$(document).ready(function()
{
	...
	*/
    ...
    if (!$.cookie('weekdeals') || $.cookie('weekdeals') === 'onsale')
    {
        $.cookie('weekdeals', 'onsale');
        $('.bestrated, .bestrated_panel').removeClass('active');
        $('.onsale, .onsale_panel').addClass('active');
    }
    if ($.cookie('weekdeals') === 'bestrated')
    {
        $('.bestrated, .bestrated_panel').addClass('active');
        $('.onsale, .onsale_panel').removeClass('active');
        $('.tabs_line span').css('left','117px').css('width', '105px');
    }

    $('.onsale, .bestrated').on('click', function () {
        let active = $(this).attr('title');
        if (active === 'onsale'){
            $.cookie('weekdeals', 'onsale');
        }
        if (active === 'bestrated'){
            $.cookie('weekdeals', 'bestrated');
        }
        location.reload();
    })
